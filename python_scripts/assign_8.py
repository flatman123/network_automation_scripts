...
#THIS SCRIPT WILL PULL COMMANDS FROM FILE ON DISK AND SAVE OUTPUT TO LOCAL MACHINE.
...
def send_commands(shell_client, cmd, line_break=enter):
    print('Building sending commands...')
    time.sleep(1)
    shell_client.send(f'terminal length 0{line_break}')
    shell_client.send(cmd + line_break)
    time.sleep(2)
...
def build_shell(client):
    ...
    with open('./commands.txt') as cmd_file:
        for line in cmd_file:
            output = send_commands(shell, line)
    return output
